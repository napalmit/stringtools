...
public class OvergripDataActivity extends Activity implements OnItemSelectedListener  {

	private TblOvergrips item = null;
	private int id = 0;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.overgrip_data);
        ...
        if(id != 0){
        	setTitle(R.string.edit_overgrip, "");
        	
        	NEW = false;
        }else{
        	setTitle(R.string.new_overgrip, "");
        	NEW = true;
        }
	...
	public void setData(){
		ArrayAdapter<TblBrands> spinnerAdapterTblBrands= new ArrayAdapter<TblBrands>(OvergripDataActivity.this,
	...
	private class SetGui extends AsyncTask<Void, Void, Void> {
	    ...
	    protected void onPreExecute() {
	        super.onPreExecute();	
	        pDialog = new ProgressDialog(OvergripDataActivity.this);
	    ...
	    protected Void doInBackground(Void... arg0) {
	    	try {
				item = function.getListOvergrips(getResources().getString(R.string.URL), id).get(0);
	...
	private class SaveData extends AsyncTask<Boolean, Void, Void> {
	    ...
	    protected Void doInBackground(Boolean... arg0) {
	    	try {
	    		if(!arg0[0])
					try {
						return_type = function.editDataOvergrip(getResources().getString(R.string.URL), item);
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
					...
					try {
						return_type = function.newOvergrip(getResources().getString(R.string.URL), item);
					} catch (JSONException e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
	    ...
	    protected void onPostExecute(Void result) {
	        ...
	        if(return_type.equals("1")){
	        	...
	        	AlertDialog.Builder builder1 = new AlertDialog.Builder(OvergripDataActivity.this);
